...
import { useFetchGetWeather } from "./repo/useWeather";
import { useEffect, useState } from "react";
import Image from "next/image";
import { TiWeatherSunny } from "react-icons/ti";
const weatherToken: string | any = process.env.NEXT_PUBLIC_WEATHER;


const Weather = () => {
	const [isLocalName, setIsLocalName] = useState<string>("");

	const codernadasWeather = {
		longitude: -50.3995,
		latitude: -22.661,
	}

	const { data } = useFetchGetWeather(codernadasWeather.longitude, codernadasWeather.latitude, weatherToken)
    function WeatherIcon(){
		switch (data?.response.weather[0].id) {
			case 800:
				return (<span className="text-[80px] text-yellow-500"><TiWeatherSunny /></span>)
		}
	}
	useEffect(() => {
		setIsLocalName(data?.response.name)
		console.log(data)
	}, [data])
	return (
		<div className="max-w-md p-8 ml-10 mx-auto rounded-lg bg-gray-900 text-gray-100">
			<div className="flex justify-between space-x-8">
				<div className="flex flex-col items-center">
					<WeatherIcon/>
					<h1 className="text-xl font-semibold">{isLocalName}</h1>
				</div>
